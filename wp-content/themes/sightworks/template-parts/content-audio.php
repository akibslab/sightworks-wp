...
$sightworks_audio_url = function_exists('get_field') ? get_field('fromate_style') : NULL;
if (is_single()) :
?>

    <article id="post-<?php the_ID(); ?>" <?php post_class('ss__post format-audio'); ?>>
        <?php if (!empty($sightworks_audio_url)) : ?>
            <div class="ss-feature__image">
                <?php echo wp_oembed_get($sightworks_audio_url); ?>
            </div>
        <?php endif; ?>
